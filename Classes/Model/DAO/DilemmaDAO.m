...
- (Dilemma *)findPersistedObject:(NSString *)key {  
    ...
    if (![key isEqualToString:@""]) {
        findPred = [NSPredicate predicateWithFormat:@"nameDilemma == %@", key];
...
- (NSArray *)retrievePersistedObjects {	
	...
	NSEntityDescription *entityAssetDesc = [NSEntityDescription entityForName:@"Dilemma" inManagedObjectContext:self.context];
    ...
    if (![self.currentKey isEqualToString:@""]) {
        ...
        NSPredicate *pred = [NSPredicate predicateWithFormat:@"nameDilemma == %@", self.currentKey];
	...
	if (self.sorts.count > 0) {
        ...
        NSSortDescriptor* sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"displayNameDilemma" ascending:YES];
