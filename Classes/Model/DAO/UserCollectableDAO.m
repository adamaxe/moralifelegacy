...
- (UserCollectable *)create {
    return [NSEntityDescription insertNewObjectForEntityForName:@"UserCollectable" inManagedObjectContext:self.context];   
}
...
- (BOOL)delete:(UserCollectable *)choice {
    NSError *error = nil;
...
- (UserCollectable *)findPersistedObject:(NSString *)key {
    ...
    if (![key isEqualToString:@""]) {
        findPred = [NSPredicate predicateWithFormat:@"collectableKey == %@", key];
...
- (NSArray *)retrievePersistedObjects {
	...
	NSEntityDescription *entityAssetDesc = [NSEntityDescription entityForName:@"UserCollectable" inManagedObjectContext:self.context];
    ...
    if (![self.currentKey isEqualToString:@""]) {
        ...
        NSPredicate *pred = [NSPredicate predicateWithFormat:@"collectableName == %@", self.currentKey];
	...
	if (self.sorts.count > 0) {
        ...
        NSSortDescriptor* sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"collectableKey" ascending:YES];
